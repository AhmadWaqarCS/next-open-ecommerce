...
import { getCategoryProducts, getSiteConfig } from "@/lib/storefront";
...
export default async function CategoryContent({
  ...
  cacheLife("max");
  const [{ category, products, total, pageCount }, config] = await Promise.all([
  ...
  return (
    <div className="page-enter">
      {/* ── Category header ──────────────────────────────────────────────── */}
      <div
        className={`${category.bg_color?.includes("bg-") ? category.bg_color : `bg-gradient-to-br ${category.bg_color ?? "from-zinc-800 to-zinc-950"}`} relative overflow-hidden`}
      >
