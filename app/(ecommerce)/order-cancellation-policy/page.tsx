...
export async function generateMetadata(): Promise<Metadata> {
  const pageRes = await getPageData("order-cancellation-policy");
  const page = pageRes?.page;
  const meta = page?.meta_info ?? {};
  return {
    title: meta.title ?? page?.title ?? "Order Cancellation Policy",
    description: meta.description ?? undefined,
  };
}

export default async function OrderCancellationPolicyPage() {
  cacheTag("page-order-cancellation-policy");
  cacheLife("max");

  const { page } = await getPageData("order-cancellation-policy");
