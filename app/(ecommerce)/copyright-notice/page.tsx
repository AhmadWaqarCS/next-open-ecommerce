...
export async function generateMetadata(): Promise<Metadata> {
  const [config, page] = await Promise.all([
    getSiteConfig(),
    getPageBySlug("copyright-notice"),
  ]);
  const meta = page?.meta_info ?? {};
  return {
    title: meta.title ?? `${page?.title ?? "Copyright Notice"} | ${config?.name ?? "Store"}`,
    description: meta.description ?? undefined,
  };
}

export default async function CopyrightNoticePage() {
  cacheTag("site-config", "site-pages", "page-copyright-notice");
  cacheLife("max");

  const page = await getPageBySlug("copyright-notice");
