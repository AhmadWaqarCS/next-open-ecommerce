...
export async function generateMetadata(): Promise<Metadata> {
  const pageRes = await getPageData("terms-and-conditions");
  const page = pageRes?.page;
  const meta = page?.meta_info ?? {};
  return {
    title: meta.title ?? page?.title ?? "Terms and Conditions",
    description: meta.description ?? undefined,
  };
}

export default async function TermsAndConditionsPage() {
  cacheTag("page-terms-and-conditions");
  cacheLife("max");

  const { page } = await getPageData("terms-and-conditions");
      ...
      <div>
        <div className="border-b border-zinc-900 bg-gradient-to-b from-zinc-900/80 to-zinc-950 pt-28 pb-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold uppercase tracking-widest text-blue-400 bg-blue-950/60 border border-blue-800/50 rounded-full mb-4">
