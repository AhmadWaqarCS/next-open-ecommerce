...
export async function generateMetadata(): Promise<Metadata> {
  const pageRes = await getPageData("legal-disclaimer");
  const page = pageRes?.page;
  const meta = page?.meta_info ?? {};
  return {
    title: meta.title ?? page?.title ?? "Legal Disclaimer",
    description: meta.description ?? undefined,
  };
}

export default async function LegalDisclaimerPage() {
  cacheTag("page-legal-disclaimer");
  cacheLife("max");

  const { page } = await getPageData("legal-disclaimer");
