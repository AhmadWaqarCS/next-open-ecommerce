...
export async function generateMetadata(): Promise<Metadata> {
  const [config, page] = await Promise.all([
    getSiteConfig(),
    getPageBySlug("return-and-refund-policy"),
  ]);
  const meta = page?.meta_info ?? {};
  return {
    title: meta.title ?? `${page?.title ?? "Return & Refund Policy"} | ${config?.name ?? "Store"}`,
    description: meta.description ?? undefined,
  };
}

export default async function ReturnAndRefundPolicyPage() {
  cacheTag("site-config", "site-pages", "page-return-and-refund-policy");
  cacheLife("max");

  const page = await getPageBySlug("return-and-refund-policy");
