    ...
    items: [
      {
        ...
        icon: (
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 v4a1 1 0 001 1m-6 0h6" />
          </svg>
        ),
      },
    ],
...
export default function SidebarLinks({ links, userRole }: SidebarLinksProps) {
  ...
  const activeSections = NAVIGATION_SECTIONS.map((section) => {
    const accessibleItems = section.items.filter((item) => {
      ...
      if (item.path === "/dashboard/secret-vault") {
  ...
  const unconfiguredAccessibleFeatures = links.filter(
    (f) => f.crud?.read && !configuredPaths.has(f.path)
  );

  if (unconfiguredAccessibleFeatures.length > 0) {
    activeSections.push({
      title: "Other",
      items: unconfiguredAccessibleFeatures.map((f) => ({
        name: f.name,
        path: f.path,
        icon: (
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="space-y-1">
            {section.items.map((item) => {
              ...
              const isActive = isHome
                ? pathname === "/dashboard"
                : pathname === item.path || Boolean(pathname?.startsWith(item.path + "/"));
