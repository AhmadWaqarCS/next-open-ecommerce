...
export const metadata: Metadata = {
  title: "Trash — Payment Methods",
  description: "Deleted payment methods",
};

export default async function DashboardPaymentMethodsTrashPage({
  searchParams,
}: {
  ...
  const userIds = paymentMethods
    .filter((m) => m.deleted_by !== null)
    .map((m) => m.deleted_by as number);
  const userNames = await resolveUserNames(userIds);

  return (
    <div className="space-y-6 flex-1 flex flex-col">
      <PaymentMethodsTrashTable
